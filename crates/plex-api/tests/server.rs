...
mod offline {
    ...
    async fn tv_library(#[future] server_anonymous: Mocked<Server>) {
        ...
        assert_eq!(
            map(&seasons, |e| e.rating_key().to_owned()),
            vec!["89", "33"]
        );
    ...
    async fn music_library(#[future] server_anonymous: Mocked<Server>) {
        ...
        assert_eq!(
            map(&artists, |e| e.rating_key().to_owned()),
            vec!["156", "142"]
        );
...
mod online {
    ...
    async fn movies(#[future] server: Server) {
        ...
        assert_eq!(
            map(&collections, |e| e.rating_key().to_owned()),
            vec!["161"]
        );
    ...
    async fn photos(#[future] server: Server) {
        ...
        assert_eq!(
            map(&albums, |e| e.rating_key().to_owned()),
            vec!["64", "51"]
        );
        assert_eq!(
    ...
    async fn find_movie(server: &Server, library_id: &str, rating_key: &str) -> [Movie; 2] {
        let library = server
            .libraries()
            .into_iter()
